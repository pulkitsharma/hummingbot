...
from .candle_spike_order_tracker import CandleSpikeOrderTracker
...
cdef class CandleSpikeStrategy(StrategyBase):
    ...
    def __init__(self,
        ...
        super().__init__()
        self._sb_order_tracker = CandleSpikeOrderTracker()
    ...
    cdef object c_get_high_price(self):
        cdef:
            ...
            object mid_price
        if self._asset_price_delegate is not None:
            mid_price = delegate.c_get_high_price()
        else:
            mid_price = self._market_info.get_mid_price()
        return mid_price

    def get_low_price(self):
        return self.c_get_low_price()
    ...
    cdef object c_get_low_price(self):
        cdef:
            ...
            object mid_price
        if self._asset_price_delegate is not None:
            mid_price = delegate.c_get_low_price()
        else:
            mid_price = self._market_info.get_mid_price()
        return mid_price
    ...
    def candle_spike_assets_df(self, to_show_current_pct: bool) -> pd.DataFrame:
    ...
    def format_status(self) -> str:
        ...
        assets_df = self.candle_spike_assets_df(not self._inventory_skew_enabled)
    ...
    cdef c_tick(self, double timestamp):
        ...
        try:
            if last_tick != current_tick and (self._filled_buys_balance>0 or self._filled_sells_balance>0):
                self._minutes_elapsed += 1
                self.logger().info(f" Minutes elapsed in order - {self._minutes_elapsed}")
    ...
    cdef object c_create_base_proposal(self):
        ...
        if self._filled_buys_balance>0:
            price = 0
            size = 0
            for limit_order_record in self._filled_buy_orders:
                price = price + (limit_order_record.price * limit_order_record.quantity)
        ...
        elif self._filled_sells_balance>0:
            price = 0
            size = 0
            for limit_order_record in self._filled_sell_orders:
                price = price + (limit_order_record.price * limit_order_record.quantity)
                size = size + limit_order_record.quantity
            price = price/size
    ...
    cdef c_did_complete_buy_order(self, object order_completed_event):
        ...
        self._filled_buy_orders.append(limit_order_record)
        self.logger().info(f"Filled buys balance : {self._filled_buys_balance}")
        if self._filled_buys_balance == self._filled_sells_balance:
            self._filled_buys_balance = self._filled_sells_balance = 0
            self._filled_buy_orders = self._filled_sell_orders = []
            self._minutes_elapsed = 0
            self.logger().info(f"Squared off buy side")
    ...
    cdef c_did_complete_sell_order(self, object order_completed_event):
        ...
        self._filled_sell_orders.append(limit_order_record)
        self.logger().info(f"Filled sells balance : {self._filled_sells_balance}")
        if self._filled_buys_balance == self._filled_sells_balance:
            self._filled_buys_balance = self._filled_sells_balance = 0
            self._filled_buy_orders = self._filled_sell_orders = []
            self._minutes_elapsed = 0
            self.logger().info(f"Squared off sell side")
